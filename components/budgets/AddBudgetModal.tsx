...
export function AddBudgetModal({ visible, onClose, editBudgetId }: AddBudgetModalProps) {
  ...
  const handleSave = () => {
    ...
    if (editBudgetId) {
      updateBudget({ id: editBudgetId, ...budgetData });
  ...
  return (
    <Modal visible={visible} transparent animationType="slide">
      <KeyboardAvoidingView 
        ...
        <View className="max-h-[85%] rounded-t-[32px] bg-background p-6 pb-12">
          
          <View className="mb-6 flex-row items-center justify-between">
            <Text variant="h2">{editBudgetId ? 'Edit Budget' : 'Add Budget'}</Text>
