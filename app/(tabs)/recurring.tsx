...
export default function RecurringScreen() {
  ...
  return (
    <View className="flex-1 bg-background" style={{ paddingTop: insets.top + 16 }}>
      <View className="px-5">
        <Header title="Recurring Expenses" showBack={true} onLeftPress={() => navigateTab('profile')} />
      </View>
      <ScrollView
        className="flex-1"
      ...
      <View className="mt-20 items-center justify-center px-6">
        <View className="mb-6 h-24 w-24 items-center justify-center rounded-full bg-gray-50 dark:bg-gray-900">
          <Icon as={CalendarClock} size={40} className="text-muted opacity-50" />
        </View>
        <Text variant="h3" className="mb-2 text-center">
          Recurring Expenses
        </Text>
        <Text className="mb-8 text-center text-muted">
          Automatically log recurring bills like rent, salaries, and utility payments.
        </Text>
