...
export default function AddSubscriptionScreen() {
  ...
  const handleSave = () => {
    ...
    if (!name.trim()) {
      Toast.show({ type: 'error', text1: 'Invalid Name', text2: 'Please enter a subscription name' });
      return;
    }
    ...
    if (hasEndDate && endDate < nextDate) {
      Toast.show({ type: 'error', text1: 'Invalid End Date', text2: 'End date must be after next billing date' });
      return;
    }
            ...
            <View className="flex-row flex-wrap gap-2">
              {(['weekly', 'monthly', 'quarterly', 'yearly'] as const).map((c) => (
                <TouchableOpacity
                  key={c}
                  onPress={() => setCycle(c)}
                  className={`flex-1 min-w-[70px] items-center justify-center rounded-full border py-2.5 px-2 ${cycle === c ? 'border-primary bg-primary' : 'border-gray-200 bg-transparent dark:border-gray-800'}`}>
                  <Text className={`text-xs font-semibold capitalize ${cycle === c ? 'text-white dark:text-black' : 'text-foreground'}`}>
                    {c}
                  </Text>
                </TouchableOpacity>
              ))}
                ...
                {categoriesList.map((cat) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      <Text className={`text-sm font-medium ${isSelected ? 'text-white dark:text-black' : 'text-foreground'}`}>
                        {cat.name}
                      </Text>
                    </TouchableOpacity>
                  );
          ...
          <View>
            ...
            <View className="flex-row gap-3">
              <TouchableOpacity
                ...
                <Text className={`text-xs font-semibold ${nextDate.toDateString() === new Date().toDateString() ? 'text-white dark:text-black' : 'text-foreground'}`}>
                  Today
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                <Icon
                  as={Calendar}
                  size={12}
                  className={nextDate.toDateString() !== new Date().toDateString() ? 'text-white dark:text-black' : 'text-foreground'}
                />
                <Text
          ...
          <View>
            ...
            {hasEndDate && (
              <View className="mt-4 flex-row justify-center">
                <TouchableOpacity onPress={() => setIsEndDatePickerOpen(true)} className="flex-row items-center gap-1.5 rounded-full border border-primary bg-primary/10 px-5 py-2.5">
                  <Icon as={Calendar} size={14} className="text-primary" />
                  <Text className="text-sm font-semibold text-primary">{formatDatePickerDate(endDate)}</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
            ...
            <TouchableOpacity
              onPress={handleSave}
              disabled={!amount.trim() || isNaN(parseFloat(amount)) || parseFloat(amount) === 0 || !name.trim()}
